...
describe('FirestoreService', () => {
  ...
  beforeEach(async () => {
    mockDoc = {
      get: jest
        .fn()
        .mockResolvedValue({ exists: true, data: () => ({ foo: 'bar' }) }),
    ...
    mockCollection = {
      ...
      get: jest
        .fn()
        .mockResolvedValue({ docs: [{ data: () => ({ foo: 'bar' }) }] }),
    } as any;
    mockFirestore = {
  ...
  it('should set document data with options', async () => {
    await expect(
      service.set('test/doc', { foo: 'baz' }, { merge: true }),
    ).resolves.toBeUndefined();
  ...
  it('should update document data', async () => {
    await expect(
      service.update('test/doc', { foo: 'updated' }),
    ).resolves.toBeUndefined();
